...
export class ScatterUtils {
    ...
    extractChar(c:number, bits:number[]) {
        let encode = 5;
        let pot = Math.pow(2, encode-1); // 16
        let value = 0;
        let index = c * encode;
        for (let i=0; i<encode; i++, pot = pot/2) {
            value += bits[index + i] * pot;
        }
        let char = this.valueToChar(value);
        return char;
    }
    ...
    decodeSlug(slug:SlugId) {
        ...
        let bits:number[] = [];
        bits = this.decodeUint64(slug.top).concat(this.decodeUint64(slug.low));
        let length = bits[250] * 32 + bits[251] * 16 + bits[252] * 8 + bits[253] * 4 + bits[254] * 2 + bits[255] * 1;
    ...
    encodeSlug(name:string):SlugId {
        ...
        for (let i=0; i<name.length; i++) {
            let value = this.charToValue(name[i]);
            this.insertChar(value, i, bits);
        }
        this.insertLength(bits, name.length);
        let slug = this.encodeUnit64(bits);
    ...
    slugTo128bits(slug:SlugId):string {
        let str = "0x";
        let topbits = this.decodeUint64(slug.top);
        let lowbits = this.decodeUint64(slug.low);
        let mixbits = [];
        for (let i=0; i<topbits.length; i++) {
            mixbits.push(topbits[i] ^ lowbits[i] ? 1 : 0);
        }
        for (let i=0; i<mixbits.length; i+=4) {
    ...
    oldEosjsEncodeName(name: string, littleEndian = false) {
        ...
        // console.debug('encodeName', name, value.toString(), ulName.toString(), JSON.stringify(bitstr.split(/(.....)/).slice(1)))
